...
import { fetchListCsv, fetchListJson, fetchListText } from "../utils/fetch";
import { createWhitelistFilter } from "../utils/filter";

const CURRENT_DOMAINS_BLACKLIST =
	"https://raw.githubusercontent.com/0x1618/disposable-email-domains/refs/heads/main/data/domains.txt";
...
async function main() {
	...
	const filtered_blacklist: string[] = Array.from(blacklist).filter(
		(domain) => psl.isValid(domain),
	);
	const blacklist_after_whitelist: string[] = filtered_blacklist.filter(
		(domain) => whitelist_filter(domain),
	);
	...
	await fs.writeFile(
		OUTPUT_DOMAIN_FILE,
		[header, ...sorted_blacklist].join("\n"),
		"utf-8",
	);
	await fs.writeFile(
		OUTPUT_DOMAIN_RAW_FILE,
		sorted_blacklist.join("\n"),
		"utf-8",
	);
	...
	await fs.writeFile(
		OUTPUT_ROOT_FILE,
		[header, ...sorted_root_blacklist].join("\n"),
		"utf-8",
	);
	await fs.writeFile(
		OUTPUT_ROOT_RAW_FILE,
		sorted_root_blacklist.join("\n"),
		"utf-8",
	);
