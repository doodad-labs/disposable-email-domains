...
export function parseListJson(json: any, key: string): string[] {
	if (key === ".") {
		if (!Array.isArray(json))
			throw new Error(`Expected JSON array but got ${typeof json}`);
		return json
			.map((item) =>
				typeof item === "string" ? standardiseDomain(item) : null,
			)
	...
	if (!Array.isArray(current))
		throw new Error(
			`Expected JSON array at key "${key}" but got ${typeof current}`,
		);
	return current
		.map((item) =>
			typeof item === "string" ? standardiseDomain(item) : null,
...
export function parseListCsv(text: string, col: number): string[] {
	...
	for (const line of lines) {
		...
		if (isNaN(col))
			throw new Error(
				`Invalid CSV key "${col}" (must be an integer index or ".")`,
			);
